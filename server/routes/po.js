...
router.post('/', async (req, res) => {
  try {
    const { wing_id, client_id, po_title, po_number, currency, exchange_rate, po_value, issue_date, expiry_date, notes } = req.body;
...
router.put('/:id', async (req, res) => {
  try {
    ...
    if (po_value    !== undefined) {
      const rate = parseFloat(exchange_rate) || 1;
      const val  = parseFloat(po_value);
      updates.po_value       = val;
      updates.exchange_rate  = rate;
      updates.pkr_equivalent = (currency === 'PKR' || !currency) ? val : val * rate;
    }
