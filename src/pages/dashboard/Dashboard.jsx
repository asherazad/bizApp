        ...
        <div className="card">
          ...
          <div className="table-wrap">
            <table className="table">
              <thead><tr><th>Bank</th><th>Account</th><th className="text-right">Balance</th></tr></thead>
              <tbody>
                {bank_balances?.length ? bank_balances.map((b, i) => (
                  <tr key={i}>
                    <td>{b.bank_name}</td>
                    <td className="text-muted">{b.account_title}</td>
                    <td className="text-right font-mono">{formatCurrency(b.current_balance, b.currency_code)}</td>
                  </tr>
                )) : <tr><td colSpan={3} className="text-muted" style={{ textAlign: 'center', padding: 20 }}>No accounts</td></tr>}
              </tbody>
            </table>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div className="card">
              ...
              <ul style={{ listStyle: 'none' }}>
                {upcoming_reminders.map((r) => (
                  <li key={r.id} style={{ padding: '10px 16px', borderBottom: '1px solid var(--border)', fontSize: 13 }}>
                    <div style={{ fontWeight: 500 }}>{r.title}</div>
                    <div className="text-muted">{formatDate(r.due_at)}</div>
                  </li>
                ))}
              </ul>
            </div>
          ...
          {upcoming_subscriptions?.length > 0 && (
            <div className="card">
              <div className="card-header">
                <h3 style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  <RefreshCw size={15} /> Upcoming Renewals
                </h3>
              </div>
              <ul style={{ listStyle: 'none' }}>
                {upcoming_subscriptions.map((s) => (
                  <li key={s.id} style={{ padding: '10px 16px', borderBottom: '1px solid var(--border)', fontSize: 13, display: 'flex', justifyContent: 'space-between' }}>
          ...
          <table className="table">
            <thead>
              <tr>
                ...
                <th>Type</th><th className="text-right">Amount (PKR)</th>
              </tr>
            </thead>
            <tbody>
              {recent_transactions?.length ? recent_transactions.map((t) => (
                <tr key={t.id}>
                  ...
                  <td className="text-right font-mono">{formatCurrency(t.pkr_amount)}</td>
                </tr>
              )) : (
                <tr><td colSpan={5} className="text-muted" style={{ textAlign: 'center', padding: 20 }}>No transactions yet</td></tr>
              )}
            </tbody>
          </table>
        </div>
