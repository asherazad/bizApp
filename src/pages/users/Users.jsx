...
function UserModal({ editUser, wings, onClose, onSaved }) {
  ...
  function toggleWing(id) {
    setForm((p) => ({
      ...p,
      wing_ids: p.wing_ids.includes(id)
        ? p.wing_ids.filter((x) => x !== id)
        : [...p.wing_ids, id],
    }));
  }
      ...
      <div className="modal" onClick={(e) => e.stopPropagation()}>
        ...
        {isEdit && (
          <div style={{ borderTop: '1px solid var(--border)', padding: '16px 24px' }}>
            ...
            >
              <KeyRound size={13} /> {showPw ? 'Cancel Password Change' : 'Change Password'}
            </button>
            ...
            {showPw && (
              <form onSubmit={submitPassword} style={{ marginTop: 12, display: 'flex', flexDirection: 'column', gap: 10 }}>
                ...
                <div className="form-group">
                  ...
                  <input type="password" className="form-control" required minLength={8} value={pwForm.confirm_password} onChange={pw('confirm_password')} placeholder="Re-enter password" />
                </div>
